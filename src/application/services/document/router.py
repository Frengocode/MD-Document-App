...
from src.application.services.document.queries.get_documents_for_participiant import (
    GetPariticipantDocuments,
)
from src.application.services.document.queries.get_documents_for_user import (
    GetUserDocuments,
)
...
@document_service_router.delete("/delete/document/{id}/", response_model=SDocument)
async def delete_document(
    ...
    service: DeleteDocument = DeleteDocument(
        verify_document=verify, repository=repository
    )
    return await service.execute(id=id, current_user=current_user)


@document_service_router.get("/get/documents/for/user/", response_model=List[SDocument])
...
@document_service_router.get(
    "/get/documents/for/participiant/", response_model=List[SDocument]
)
async def get_documents_for_participant(
    ...
    service: GetPariticipantDocuments = GetPariticipantDocuments(
        repository=repository, get_user=get_user
    )
    return await service.execute(offset=offset, limit=limit, current_user=current_user)


@document_service_router.get("/get/document/{id}/", response_model=SDocument)
async def get_document(
    ...
    service: GetDocument = GetDocument(
        repository=repository, verify_authors=verify_authors, get_user=get_user
    )
    return await service.execute(id=id, current_user=current_user)
...
async def get_document_file(
    filename: Annotated[str, Query(...)], minio: Annotated[MinIO, Depends(get_minio_s3)]
) -> StreamingResponse:
...
@document_service_router.get("/get/documents/", response_model=List[SDocument])
