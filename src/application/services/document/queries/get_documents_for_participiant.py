...
class GetPariticipantDocuments(BaseService):
    def __init__(
        self, repository: ABCDocumentRepository, get_user: BaseService
    ) -> None:
        self.repository = repository
        self.get_user = get_user

    async def execute(
        self, offset: int, limit: int, current_user: SUser
    ) -> List[SDocument]:

        documents: List[Document] = await self.repository.get_all(
            offset=offset, limit=limit, participiant_id=current_user.id
        )
        if not documents:
        ...
        users: List[SUser] = await asyncio.gather(
            *[
                self.get_user.execute(user_id=document.user_id)
        ...
        return [
            SDocument(**document.__dict__, user=user)
